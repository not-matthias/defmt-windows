...
use stub::{BitflagsKey, StringEntry, Table, TableEntry, Tag, DEFMT_VERSIONS};
use anyhow::{anyhow, bail, ensure};
use object::{File, Object, ObjectSection, ObjectSymbol};
use pdb::{FallibleIterator, PDB};

fn get_version_and_encoding(pe: &File, pdb: &mut PDB<Cursor<&[u8]>>) -> anyhow::Result<(Option<String>, Option<String>)>  {
    ...
    while let Some(symbol) = symbols.next()? {
        let Ok(symbol) = symbol.parse() else {
            continue;
        };
        ...
        if let Some(new_encoding) = try_get_encoding(name.as_ref()) {
            ...
            log::info!("encoding: {:?}", new_encoding);
...
pub fn parse_impl(file: &[u8], pdb: &[u8], check_version: bool) -> Result<Option<Table>, anyhow::Error> {
    let pe = object::File::parse(file)?;
    let mut pdb = PDB::open(Cursor::new(pdb))?;

    let (version, encoding) = get_version_and_encoding(&pe, &mut pdb)?;
    ...
    let defmt_section = pe.section_by_name(".defmt._");
    ...
    for entry in pe.symbols() {
        ...
        if name.starts_with("_defmt._") || name.starts_with("__DEFMT_MARKER") {
